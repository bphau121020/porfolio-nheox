(() => {
  ...
  const detectTablet = (): void => {
    ...
    const setViewport = (): void => {
      ...
      if (screenWidth < 375 && portrait) {
        ...
        (screenWidth >= 768 && screenWidth <= 1199) ||
  ...
  const smoothScroll = (): void => {
    ...
    const triggerScroll = (context: string | HTMLAnchorElement): boolean => {
      try {
        let href: string;
        let jContext: JQuery;
        ...
        if (typeof context === 'string') {
          ...
          jContext = $(context);
          href = '#' + (jContext.attr('href')?.split('#')[1] || '');
        }
        ...
        if (!hasScrollClass && target.length) {
          const position = (target.offset()?.top || 0) - headerHeight;
